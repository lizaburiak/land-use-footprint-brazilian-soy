...
|---|---|---|---|---|
| 1 | `RailCargo_2006-21_ANTT.xls` | step 06 (multimode transport) | optional path | ✅ **RESOLVED 2026-07-27** — raw OD CSVs 2006–2023 downloaded to `data/geo/ANTT/RailCargo_od/` (via Wayback; see README there). Step 06 needs a small reader change (xls → CSV). |
| 2 | `ip_add.gpkg` | step 06 | optional path | ✅ **REBUILT 2026-07-27** — `code/prep/prep_ip_add.R` regenerated `data/geo/ANTAQ/ip_add.gpkg` (6 ports, EPSG:5880, IP.shp schema + co_mun). Verified: rbind-compatible with step 06, all ports on waterway (0 m) and on 5-km water raster pixels; 2017–2022 soy cargo now fully matched except catch-all `BR200`. Stefan's original remains unrecovered (not on GitHub/Zenodo) but is no longer needed for these years. |
| 3 | `train_stations_soy.gpkg` | step 06 | optional path | ⚠️ **not actually needed** — `stations_man` is read at `06_transport_cost.R:63` and never used afterwards (verified in Stefan's original too); comment out the `st_read` line |
| 4 | ANTAQ cargo for years ≤ 2016 (`2013Carga*.txt`) | step 06 for early years | optional path | ⏳ ANTAQ open data covers 2010+ (`web3.antaq.gov.br/ea/txt/{YEAR}Carga.zip`), but the server was **down (502) on 2026-07-27** and the year zips are not in the Wayback Machine — retry the URL later |
...
## 2. Multimode transport (step 06 → Python `transport_lp` / GAMS) — ✅ RUNS (2019 validated 2026-07-28)

> **UPDATE 2026-07-28:** the full multimode chain ran end-to-end for 2019 on this laptop:
> step 06 (~40 min) → `transport_lp/solve_one.py` (HiGHS, optimal, ~2 h) → 08 → 10 → 11.
> Fixes required (all committed to the pipeline): rail CSV reader replacing the lost xls;
> GDAL segfault on the OSM gpkg write (free memory + write only the raster attribute);
> positional station-column indexing; MUN_capitals/SOY_MUN row alignment; auto-snap of
> off-raster ports (BRPA005 carried 5% of water tonnage); dynamic bootstrap-column
> detection in 08/10 (Stefan hardcoded "00001"). **Result vs TRASE (2019, global pooled
> Pearson): multimode r=0.728 vs Euclidean r=0.723** — statistically equivalent, so the
> Euclidean default remains justified; multimode helps for CHN/KOR/FRA, hurts for NLD/DEU.
> Note: central solve only — Stefan's 1000-iteration bootstrap costs ~2 h/solve here.
...
|---|---|---|
| `RailCargo_2006-21_ANTT.xls` | `data/geo/` | ✅ **replaced by raw CSVs** (2026-07-27): `data/geo/ANTT/RailCargo_od/producao_origem_destino_{2006..2023}.csv`, ANTT SAFF OD data fetched through the Wayback Machine (dados.antt.gov.br WAF blocks non-BR IPs). 2006–2022 complete, 2023 partial. Format notes + required step-06 reader change in `data/geo/ANTT/RailCargo_od/README.md`. |
| `ip_add.gpkg` | `data/geo/ANTAQ/` | ✅ **REBUILT** (2026-07-27) by `code/prep/prep_ip_add.R`. Stefan's hand-geocoded original is unrecoverable (not in `strsek/soyprint` — code-only repo — nor Zenodo); the gap it patched is **0–2.5% of interior soy tonnage/yr** for 2017–2022 (0.42 Mt worst case, 2018; measured by `code/analysis/check_missing_ports.R`). The rebuild geocodes the 6 terminals identified via `Instalacao_Origem/Destino.txt` — `BRSP020` TNPM Anhembi (Tietê/SP), `BRSP021` TNPM Pederneiras (Tietê/SP), `BRAM076` Coari Pequi (Solimões/AM), `BRPHA` Prainha (Amazonas/PA), `BRRO016` TEPOVEL (Madeira/RO), `BRJUR` Juruti (Amazonas/PA) — by snapping each to the DNIT waterway inside its municipality nearest the municipal seat. Verified step-06-style: rbind schema match, 0 m to waterway, all on 5-km water raster pixels; 2017–2022 soy cargo fully matched except `BR200` "Terminais Interiores" (a non-geographic catch-all — unmappable for Stefan too). ⚠️ If steps 06/07 are later run for **≤ 2016**, re-run `check_missing_ports.R` on that year's cargo once obtained — Stefan's 2013 port list may contain additional terminals. |
| `train_stations_soy.gpkg` | `data/geo/ANTT/` | **Stefan's archive** — hand-curated soy rail stations. Same search result as above; ask Stefan/WU. **Confirmed unused**: `06_transport_cost.R:63` loads it into `stations_man`, which is never referenced again (true in Stefan's original too) — safe to comment out the `st_read`, no data needed. |

Plus, ANTAQ waterway cargo for the target year:
- 2017–2022: ✅ already present (`{YEAR}Carga.txt` + `_Conteinerizada.txt`)
- ≤ 2016: ⏳ ANTAQ publishes 2010+ at `web3.antaq.gov.br/ea/txt/{YEAR}Carga.zip`, but the server
  returned 502 on 2026-07-27 and these zips are not archived in the Wayback Machine. Retry later;
  if it stays down, the interactive EA portal (Qlik) or an ANTAQ data request are the fallbacks.
  For 2000–2009 there is no ANTAQ series at all (Stefan used his 2013 download as proxy).
