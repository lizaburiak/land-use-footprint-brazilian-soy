...
GEO_states <- st_read("data/old/geo/GADM_boundaries/gadm36_BRA_1.shp", stringsAsFactors = FALSE) %>%  
  mutate(nm_state = substr(HASC_1, 4,5))
comp_list <- readRDS( "results/intermediate/comp_list.rds")
MUN_capital_dist <- readRDS("results/intermediate/MUN_capital_dist.rds")
class(MUN_capital_dist) <- "numeric"
EXP_NAT_wide <- readRDS("results/intermediate/EXP_NAT_wide.rds")
