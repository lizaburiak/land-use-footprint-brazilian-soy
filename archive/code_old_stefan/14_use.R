...
regions <- fread("data/old/FABIO/inst/regions_full.csv")
items <- fread("data/old/FABIO/inst/items_full.csv")

cbs <- readRDS("results/intermediate/FABIO/cbs_full.rds") 
sup <- readRDS("results/intermediate/FABIO/sup.rds")

use_items <- fread("data/old/FABIO/inst/items_use.csv")
...
tcf_cbs <- fread("data/old/FABIO/inst/tcf_cbs.csv")
...
eth_tcf <- fread("data/old/FABIO/inst/tcf_eth.csv")
...
# Use animal stocks
live <- readRDS("data/old/FABIO/tidy/live_tidy.rds")
...
conv_k <- fread("data/old/FABIO/inst/conv_krausmann.csv")
...
conv_b <- fread("data/old/FABIO/inst/conv_bouwman.csv")
conc_b <- fread("data/old/FABIO/inst/conc_bouwman.csv")
...
bean_feed_mun <- readRDS("results/intermediate/bean_feed_t.rds")
cake_feed_mun <- readRDS("results/intermediate/cake_feed_t.rds")
...
#saveRDS(feed_mun, "results/intermediate/FABIO/feed_mun.RDS") # just for sanity checks
...
opt_tcf <- fread("data/old/FABIO/inst/tcf_optim.csv")
opt_in <- fread("data/old/FABIO/inst/optim_in.csv")
opt_out <- fread("data/old/FABIO/inst/optim_out.csv")
...
if (optimize) {
  ...
  if (write) saveRDS(results, paste0("./results/intermediate/FABIO/optim_results_",Sys.Date(),".rds"))

  } else {
  results <- readRDS("results/intermediate/FABIO/optim_results_2021-11-04.rds")
}
...
if (write){
  saveRDS(cbs, "results/intermediate/FABIO/cbs_final.rds")
  saveRDS(use, "results/intermediate/FABIO/use_final.rds")
  saveRDS(use_fd, "results/intermediate/FABIO/use_fd_final.rds")
  saveRDS(sup, "results/intermediate/FABIO/sup_final.rds")
}
