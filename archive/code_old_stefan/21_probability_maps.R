...
GEO_MUN_SOY <- readRDS("results/intermediate/GEO_MUN_SOY_fin.rds") %>% st_transform(crs = 4326)
GEO_states <- st_read("data/old/geo/GADM_boundaries/gadm36_BRA_1.shp", stringsAsFactors = FALSE) %>% st_transform(crs = 4326)
...
tile_names = list.files("data/old/geo/mb_tiles/",pattern="^.*.tif$")
tile_paths = paste0("data/old/geo/mb_tiles/",tile_names)
