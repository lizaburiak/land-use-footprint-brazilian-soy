...
btd <- readRDS("data/old/FABIO/FABIO_exp/v1/btd_full.rds") 
cbs <- readRDS("results/intermediate/FABIO/cbs_full.rds") 
sup <- fread("data/old/FABIO/inst/items_supply.csv")
...
shares <- fread("data/old/FABIO/inst/items_supply-shares.csv")
...
# fill in milk prices
prices <- readRDS("data/old/FABIO/tidy/prices_tidy.rds")
...
if(write){
  saveRDS(sup, "results/intermediate/FABIO/sup.rds")
}
